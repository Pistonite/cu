...
static PRINT_LEVEL: AtomicU8<PrintLevel> = AtomicU8::new(PrintLevel::Normal as u8);
...
pub fn init_print_options(color: ColorLevel, level: PrintLevel, prompt: Option<PromptLevel>) {
    ...
    {
        let _ = prompt;
        PROMPT_LEVEL.set(PromptLevel::No);
    }
    
    PRINT_LEVEL.set(level);
    struct LogImpl;
    impl log::Log for LogImpl {
        fn enabled(&self, metadata: &log::Metadata) -> bool {
            match GLOBAL_LOG_FILTER.get() {
                Some(filter) => filter.enabled(metadata),
                None => Lv::from(metadata.level()).can_print(PRINT_LEVEL.get()),
            }
        }
